...
class PyPiParser(BaseParser):
    ...
    def extract_vcs_repo(self, metadata):
        ...
        for label, url in project_urls.items():
            if any(vcs in url.lower() for vcs in ["github.com", "gitlab.com", "bitbucket.org"]):
                return url
        
        # Check the 'home_page' field
        home_page = metadata.get('info', {}).get('home_page', '')
        if home_page and any(vcs in home_page.lower() for vcs in ["github.com", "gitlab.com", "bitbucket.org"]):
            return home_page
        elif home_page:
        ...
        if any(vcs in download_url.lower() for vcs in ["github.com", "gitlab.com", "bitbucket.org"]):
            return download_url

        # Check the package description for VCS links (less reliable)
        description = metadata.get('info', {}).get('description', '')
        if any(vcs in description.lower() for vcs in ["github.com", "gitlab.com", "bitbucket.org"]):
    ...
    def scrape_homepage_for_vcs(self, homepage_url):
        ...
        try:
            ...
            for a_tag in soup.find_all('a', href=True):
                ...
                if any(vcs in href for vcs in ['github.com', 'gitlab.com', 'bitbucket.org']):
                    return a_tag['href']
